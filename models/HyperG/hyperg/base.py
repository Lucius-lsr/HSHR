...
class HyperG:
    def __init__(self, H, X=None, w=None, u=None):
        ...
        if u is not None:
            self.u = u.reshape(-1)
            assert self.u.shape[0] == self._n_nodes
        else:
            self.u = np.ones(self._n_nodes)
    ...
    def node_weighted_theta(self):
        U = sparse.diags(self.u, shape=(self._n_nodes, self._n_nodes))
        DV2 = self.inv_square_node_degrees()
        H = self._H.tocsr()
        W = sparse.diags(self.w)

        # cal INVDE
        de = sparse.csr_matrix.dot(self.u.reshape(1, -1), H).reshape(-1)
        inv_de = np.power(de, -1.)
        INVDE = sparse.diags(inv_de, shape=(self._n_edges, self._n_edges))
        THETA = DV2.dot(U).dot(H).dot(W).dot(INVDE).dot(H.T).dot(U).dot(DV2)

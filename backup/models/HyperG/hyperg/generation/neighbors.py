...
def gen_knn_hg(X, n_neighbors, prob=1, is_prob=True, with_feature=False):
    """
...
def gen_nw_knn_hg(X, n_neighbors, u, prob=1, is_prob=True, with_feature=False):
    """
    :param X: numpy array, shape = (n_samples, n_features)
    :param n_neighbors: int,
    :param is_prob: bool, optional(default=True)
    ...
    assert u.shape[0] == H.shape[0]

    if with_feature:
        return HyperG(H, w=w, X=X, u=u)

    return HyperG(H, w=w, u=u)

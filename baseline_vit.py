...
if __name__ == '__main__':
    ...
    model = get_moco(
        ViT(num_patches=2000, num_classes=128, dim=512, pos_dim=24, depth=2, heads=4, mlp_dim=128, dropout=0.5),
        ViT(num_patches=2000, num_classes=128, dim=512, pos_dim=24, depth=2, heads=4, mlp_dim=128, dropout=0.5),
        device,
        feature_out)
    ...
    for epoch in range(500):
        ...
        for feature_0, x_idx_0, y_idx_0, feature_1, x_idx_1, y_idx_1, path in train_dataloader:
            ...
            output, target = model(im_q=(feature_0, x_idx_0, y_idx_0), im_k=(feature_1, x_idx_1, y_idx_1))
        ...
        for feature_0, x_idx_0, y_idx_0, feature_1, x_idx_1, y_idx_1, path in test_dataloader:
            ...
            feature_1, x_idx_1, y_idx_1 = feature_1.to(device), x_idx_1.to(device), y_idx_1.to(device)
            with torch.no_grad():
                f_0 = model.encoder_q((feature_0, x_idx_0, y_idx_0)).cpu().detach().numpy()
                f_1 = model.encoder_q((feature_1, x_idx_1, y_idx_1)).cpu().detach().numpy()

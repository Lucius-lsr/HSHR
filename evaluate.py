...
class Evaluator:
    ...
    def add_patches(self, patches, paths, weights=None):
        ...
        for i in range(patches.shape[0]):
            if weights is None:
                ...
                self.add_patch(patches[i], paths[i], weights[i])

    def add_patch(self, patch, path, weight=None):
        assert len(patch.shape) == 2
        for j in range(patch.shape[0]):
            if weight is None:
                self.result_dict[path + '@' + str(j)] = (patch[j], 1)
            else:
                self.result_dict[path + '@' + str(j)] = (patch[j], weight[j])
    ...
    def fixed_report_patch(self, k=10, alpha=1, beta=1, types=None, inc=None, list_slide_id=None):
        ...
        slide_top_idx = hyedge_similarity(inc, alpha, beta)
        mMV, cm = mmv_accuracy(5, list_slide_id, slide_top_idx)
        print(mean_acc(mMV), end=' ')
        return inc, list_slide_id
